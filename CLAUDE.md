...
Two-sided haircut app: Flask JSON API (`lineup_backend/`, entry `app.py`) and a
...
JWTs from `POST /auth/dev-login` when Firebase is not configured), pick a role
once at onboarding, and spend credits on metered routes; barbers can hold a
Pro plan. README.md has the architecture, env vars, API table with auth and
credit columns, production setup and deployment details.
...
sign-in. Seeded accounts: `client@lineup.dev` (uid `client_1`, 3 credits) and
`barber@lineup.dev` (uid `barber_1`, shop "Mike's Cuts", free plan). Copy
`.env.example` to `.env` to add keys; set `LINEUP_DEV_SECRET` so dev tokens
survive restarts. **Restart the API on :5000 after any backend change**; a
stale server answers with the old routes and auth rules.

## Test and verify

- Backend: `.venv/bin/python -m pytest -q` (159 tests, no network; fakes are
  injected via `create_app(**overrides)`, see `tests/conftest.py`). Add tests
...
- Frontend: there is no JS test suite. Verify in a headless browser (Playwright
  ...
  and 1440 px, signed out and as both seeded accounts, with zero console errors.
...
- `lineup_backend/routes/*.py`: one blueprint per domain, registered in
  `routes/__init__.py`. `services/`: integrations with mock fallbacks, plus
...
- `src/js/main.js` boots every module; `router.js` (views and auth gate),
  `session.js` (token + user in localStorage), `auth.js` (Firebase or dev
  sign-in, onboarding, account menu), `billing.js` (pricing, credits pill, Pro
  locks, Checkout/portal, usage, 402 modal), `api.js` (the only fetch wrapper),
  `state.js` (prefs, last analysis; ids are the uid), `nav.js` (app tabs),
  `ui/` (toast, modal, confirm, skeleton, dropzone), `features/` (one module
  per tab or modal).
...
- Error codes the frontend handles globally: 401 (`unauthorized`,
  `invalid_token`, `token_expired`) clears the session; 403
  `onboarding_required` routes to `#/onboarding`; 402 `insufficient_credits`
  opens the credits modal, 402 `pro_required` shows the upgrade CTA; 429
  `daily_cap_reached`; 503 `stripe_not_configured` / `auth_not_configured`.
  Keep these codes stable.
