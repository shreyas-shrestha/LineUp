...
// --- Enhancements appended (non-breaking overrides) ---
function wireEnhancements() {
  // Onboarding
  const dismissed = localStorage.getItem('lineup_onboarding_dismissed') === '1';
  const banner = document.getElementById('onboarding-banner');
  if (banner && !dismissed) banner.classList.remove('hidden');
  const dismissBtn = document.getElementById('dismiss-onboarding');
  if (dismissBtn) dismissBtn.onclick = () => { localStorage.setItem('lineup_onboarding_dismissed','1'); banner?.classList.add('hidden'); };
  ...
  if (priceSel) priceSel.onchange = () => { exploreFilters.price = priceSel.value; applyExploreFilters(); };
  if (ratingSel) ratingSel.onchange = () => { exploreFilters.rating = parseFloat(ratingSel.value||'0'); applyExploreFilters(); };
  if (openNowChk) openNowChk.onchange = () => { exploreFilters.openNow = openNowChk.checked; applyExploreFilters(); };
  if (viewListBtn) viewListBtn.onclick = () => { exploreFilters.view='list'; document.getElementById('barber-map')?.classList.add('hidden'); document.getElementById('barber-list-container')?.classList.remove('hidden'); };
  if (viewMapBtn) viewMapBtn.onclick = () => { exploreFilters.view='map'; document.getElementById('barber-list-container')?.classList.add('hidden'); document.getElementById('barber-map')?.classList.remove('hidden'); renderBarberMap(window._lastBarbers||[]); };

  // ESC closes modals
  window.addEventListener('keydown', (e) => { if (e.key==='Escape') [document.getElementById('add-post-modal'), document.getElementById('upload-work-modal'), document.getElementById('book-appointment-modal'), document.getElementById('virtual-tryon-modal')].forEach(m=> m && !m.classList.contains('hidden') && m.classList.add('hidden')); });
}

function applyExploreFilters() {
  if (window._lastBarbers) {
    renderRealBarberList(applyFilters(window._lastBarbers), true);
    if (exploreFilters.view === 'map') renderBarberMap(applyFilters(window._lastBarbers));
  }
}

function applyFilters(barbers) {
  return barbers.filter(b => {
    if (exploreFilters.rating && (b.rating||0) < exploreFilters.rating) return false;
    if (exploreFilters.openNow && b.open_now === false) return false;
    if (exploreFilters.price !== 'any') {
      const level = inferPriceLevel(b.avgCost);
      if (level !== exploreFilters.price) return false;
    }
    return true;
  });
}

function inferPriceLevel(avgCost) {
  const s = String(avgCost||'').toLowerCase();
  if (s.includes('$$$') || s.match(/\$?\d{2}/) && parseInt(s.match(/\d{2}/)[0],10)>=60) return '$$$';
  if (s.includes('$$') || s.includes('45') || s.includes('50')) return '$$';
  return '$';
}

// Override to capture daily reset
const _origTest = testBackendConnection;
async function testBackendConnection() {
  try {
    const resp = await fetch(`${API_URL}/health`);
    const data = await resp.json();
    window.lastHealth = data;
    console.log('✅ Backend connected:', data);
  } catch { console.log('⚠️ Backend may be sleeping. Will wake up on first request.'); }
}

// Override loadNearbyBarbers with caching + filters + map support
async function loadNearbyBarbers(location = 'Atlanta, GA', recommendedStyles = []) {
  if (barberListContainer) {
    barberListContainer.innerHTML = '<div class="text-center py-8"><div class="loader mx-auto"></div><p class="mt-4 text-gray-400">Finding real barbershops near you...</p></div>';
  }
  try {
    ...
    const data = await response.json();
    if (data.barbers && data.barbers.length>0) {
      localStorage.setItem(`barbers_cache_${location}`, JSON.stringify(data));
      window._lastBarbers = data.barbers;
      renderRealBarberList(applyFilters(data.barbers), data.real_data);
      if (exploreFilters.view === 'map') renderBarberMap(applyFilters(data.barbers));
    } else {
      throw new Error('No barbershops found');
    }
  } catch (e) {
    const cached = localStorage.getItem(`barbers_cache_${location}`);
    if (cached) {
      try { const data = JSON.parse(cached); window._lastBarbers=data.barbers; renderRealBarberList(applyFilters(data.barbers), false); if (exploreFilters.view==='map') renderBarberMap(applyFilters(data.barbers)); return; } catch {}
    }
    if (barberListContainer) barberListContainer.innerHTML = '<div class="bg-red-900/20 border border-red-500/50 rounded-lg p-4 text-center"><p class="text-red-400">Unable to load barbershops. Please check your location and try again.</p></div>';
  }
}

// Override renderRealBarberList to add reset info and next slot chip
function renderRealBarberList(barbers, isRealData=false) {
  if (!barberListContainer) return;
  const dataSourceBadge = isRealData ? '<span class="bg-green-500/20 text-green-400 px-3 py-1 rounded-full text-xs">✓ Real Barbershops</span>' : '<span class="bg-yellow-500/20 text-yellow-400 px-3 py-1 rounded-full text-xs">Sample Data</span>';
  const resetInfo = window.lastHealth && window.lastHealth.api_usage ? `<span class="text-xs text-gray-400">Daily reset: ${window.lastHealth.api_usage.daily_reset}</span>` : '';
  barberListContainer.innerHTML = `
    <div class="mb-4 p-4 bg-gray-800/50 rounded-xl border border-gray-700">
      <div class="flex justify-between items-center mb-2">
        <p class="text-lg font-semibold text-white">Found ${barbers.length} barbershops</p>
        <div class="flex items-center gap-2">${resetInfo}${dataSourceBadge}</div>
      </div>
      <p class="text-sm text-gray-400">These barbers specialize in the styles you're looking for</p>
    </div>
  `;
  barbers.forEach(barber => {
    ...
    if (barber.hours && barber.hours.length>0) {
      const today=new Date().getDay();
      const todayHours = barber.hours[today===0?6:today-1];
      hoursHtml = `<p class="text-xs text-gray-500 mt-1">${todayHours || 'Hours not available'}</p>`;
    }
    let openStatus='';
    if (barber.open_now !== null && barber.open_now !== undefined) {
      openStatus = barber.open_now ? '<span class="bg-green-500/20 text-green-400 px-2 py-1 rounded text-xs">Open Now</span>' : '<span class="bg-red-500/20 text-red-400 px-2 py-1 rounded text-xs">Closed</span>';
    }
    const ratingText = barber.user_ratings_total ? `${barber.rating} ★ (${barber.user_ratings_total} reviews)` : `${barber.rating} ★`;
    card.innerHTML = `
      <div class="flex flex-col sm:flex-row">
        ${barber.photo ? `<img src="${barber.photo}" alt="${barber.name}" class="w-full sm:w-48 h-48 sm:h-auto object-cover" loading="lazy" onerror="this.src='https://placehold.co/400x300/1a1a1a/38bdf8?text=Barbershop'">` :
        `<div class="w-full sm:w-48 h-48 sm:h-auto bg-gray-800 flex items-center justify-center">
          <svg class="w-16 h-16 text-gray-600" fill="currentColor" viewBox="0 0 24 24"><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/></svg>
        </div>`}
        <div class="p-5 flex-1">
          <div class="flex justify-between items-start mb-2">
            <div class="flex-1">
          ...
          <div class="flex items-center gap-4 mb-3">
            <span class="text-yellow-400 flex items-center gap-1">${ratingText}</span>
            <span class="text-green-400 font-semibold">~${barber.avgCost||''}</span>
            <span class="text-xs bg-gray-700 text-gray-300 px-2 py-1 rounded">${nextAvailableSlot()}</span>
            ${barber.phone && barber.phone!=='Call for info' ? `<a href="tel:${barber.phone}" class="text-sky-400 hover:text-sky-300 text-sm">${barber.phone}</a>` : '<span class="text-gray-500 text-sm">No phone listed</span>'}
          </div>
          <div class="flex flex-wrap gap-2 mb-4">${(barber.specialties||[]).map(s=>`<span class="bg-sky-500/20 border border-sky-500/50 text-sky-300 text-xs px-3 py-1 rounded-full">${s}</span>`).join('')}</div>
          <div class="flex gap-2">
            <button onclick="openBookingModal('${barber.id}','${barber.name.replace(/'/g, "\\'")}')" class="bg-green-500 text-white px-6 py-2 rounded-lg hover:bg-green-600 transition-colors">Book Appointment</button>
            ${barber.website ? `<a href="${barber.website}" target="_blank" class="bg-gray-700 text-white px-6 py-2 rounded-lg hover:bg-gray-600 transition-colors">Website</a>` : ''}
            ${barber.location ? `<a href="https://www.google.com/maps/dir/?api=1&destination=${barber.location.lat},${barber.location.lng}" target="_blank" class="bg-blue-500 text-white px-6 py-2 rounded-lg hover:bg-blue-600 transition-colors">Directions</a>` : ''}
          </div>
        </div>
      </div>`;
    barberListContainer.appendChild(card);
  ...
  const tipsDiv = document.createElement('div');
  tipsDiv.className='mt-6 p-4 bg-gray-800/50 rounded-lg border border-gray-700';
  tipsDiv.innerHTML = `<p class="text-sm text-gray-400 mb-2">💡 Search Tips:</p><ul class="text-xs text-gray-500 space-y-1"><li>• Enter your ZIP code for more precise results (e.g., "30308")</li><li>• Use neighborhood names (e.g., "Buckhead, Atlanta")</li><li>• Include city and state (e.g., "Atlanta, GA")</li></ul>`;
  barberListContainer.appendChild(tipsDiv);
}

// Wrap tryOn to count usage
const _origTryOnStyle = tryOnStyle;
async function tryOnStyle(styleName) {
  incrementTryOnCount(styleName);
  return _origTryOnStyle(styleName);
}

function nextAvailableSlot() {
  const now = new Date();
  const next = new Date(now.getTime()+60*60*1000);
  return `Next: ${next.toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})}`;
}

function generateICSLink(apt) {
  const dt = new Date(`${apt.date}T${apt.time}`);
  const end = new Date(dt.getTime()+45*60000);
  const pad=(n)=>String(n).padStart(2,'0');
  const fmt=(d)=>`${d.getUTCFullYear()}${pad(d.getUTCMonth()+1)}${pad(d.getUTCDate())}T${pad(d.getUTCHours())}${pad(d.getUTCMinutes())}00Z`;
  const ics=['BEGIN:VCALENDAR','VERSION:2.0','PRODID:-//LineUp//EN','BEGIN:VEVENT',`DTSTART:${fmt(dt)}`,`DTEND:${fmt(end)}`,`SUMMARY:Haircut Appointment`,`DESCRIPTION:${apt.service} with ${apt.barberName}`,'END:VEVENT','END:VCALENDAR'].join('\n');
  return 'data:text/calendar;charset=utf8,'+encodeURIComponent(ics);
}

function rebook(barberId, barberName, service) {
  openBookingModal(barberId||'rebook', barberName||'Selected Barber');
  const map={'Haircut ($45)':'haircut','Haircut + Beard ($65)':'haircut-beard','Beard Trim ($25)':'beard-only'};
  const sel=document.getElementById('appointment-service');
  if (sel) sel.value = map[service] || '';
}

function trackEvent(name, data={}) {
  try { console.log('[analytics]', name, data); if (window.firebaseService && window.firebaseService.track) window.firebaseService.track(name, data); } catch {}
}
