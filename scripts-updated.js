// scripts-updated.js - Fixed Frontend JavaScript for LineUp Two-Sided Platform
...
const mockSocialPosts = [
  ...
  },
  {
    ...
    caption: 'Before and after transformation! Beard trim + fade combo 🔥',
    likes: 89,
    timeAgo: '3d',
    liked: true
  }
...
const mockBarberPortfolio = [
  ...
  },
  {
    ...
    image: 'https://images.unsplash.com/photo-1599351431202-1e0f0137899a?w=400&h=400&fit=crop',
    description: 'Short textured crop with natural volume. Low maintenance style.',
    likes: 25,
    date: '2024-01-13'
  },
  {
    ...
    image: 'https://images.unsplash.com/photo-1605497788044-5a32c7078486?w=400&h=400&fit=crop',
    description: 'Dramatic undercut with longer top for versatile styling.',
    likes: 31,
    date: '2024-01-12'
  },
  {
    id: 5,
    styleName: 'Beard Sculpting',
    image: 'https://images.unsplash.com/photo-1580618672591-eb180b1a973f?w=400&h=400&fit=crop',
    description: 'Professional beard trim and line-up. Sharp and clean.',
    likes: 20,
    date: '2024-01-11'
  }
];
...
class LineUpVirtualTryOn {
  ...
  async startTryOn() {
    ...
    if (!photoElement || !imagePreview || !imagePreview.src) {
    ...
    try {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          if (response.status === 503 && errorData.setup_instructions) {
            alert('⚠️ HairFastGAN Not Set Up Yet!\n\nSee HAIRFAST_SETUP.md for setup instructions.');
      ...
      if (!error.message.includes('not configured')) {
        alert(`Failed to transform hairstyle.\n\nError: ${error.message}`);
      }
    } finally {
      const loading = document.getElementById('tryon-loading');
      if (loading) loading.classList.add('hidden');
...
window.loadSubscriptionPackages = loadSubscriptionPackages;berName: 'Mike\'s Cuts',
    date: '2024-01-20',
    time: '14:00',
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  setTimeout(async () => {
    ...
    if (firebaseService) {
      ...
      // Load real data from Firebase (will override mock data)
...
function setupEventListeners() {
  ...
  // Barber modals
  uploadWorkButton.addEventListener('click', () => {
    uploadWorkModal.classList.remove('hidden');
  ...
  if (viewScheduleBtn) {
    viewScheduleBtn.addEventListener('click', () => switchTab('barber-schedule'));
  }
...
function switchTab(targetTab) {
  ...
  // Render specific content when navigating to tabs
...
function toggleLike(postId) {
  ...
  if (firebaseService) {
    firebaseService.toggleLike(postId).then(result => {
      if (result.success) {
...
function renderBarberPortfolio() {
  ...
  barberPortfolio.forEach(work => {
    ...
    workElement.className = 'bg-gray-900/50 border border-gray-700 rounded-2xl overflow-hidden hover:border-sky-500/50 transition-all';
...
async function submitPortfolioWork() {
  ...
  alert('Portfolio work uploaded successfully! 🎨');
}
    ...
    if (barberListContainer) {
      barberListContainer.innerHTML = `
        <div class="bg-red-900/20 border border-red-500/50 rounded-lg p-4 text-center">
          <p class="text-red-400">Unable to load barbershops. Please try again.</p>
        </div>
      `;
    }
  }
...
function findMatchingBarbers() {
  ...
  if (barberIntro) {
    barberIntro.innerHTML = `
      <span class="text-gray-300">Finding barbershops in</span> 
...
function findBarbersForStyle(styleName) {
  ...
  console.log(`Finding barbers for: ${styleName} in ${location}`);
...
function renderBarberAppointments() {
  ...
  appointments.forEach(appointment => {
    ...
    appointmentElement.innerHTML = `
      ...
      ${appointment.notes !== 'No special requests' ? `<p class="text-gray-400 text-sm mt-3">Notes: ${appointment.notes}</p>` : ''}
    `;
    barsm font-medium text-gray-300">${item.label}</p>
      </div>
